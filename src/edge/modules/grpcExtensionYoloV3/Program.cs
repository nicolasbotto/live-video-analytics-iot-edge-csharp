...
namespace grpcExtension
{
    public sealed class Program : IHostedService, IDisposable
    {
        ...
        public Task StartAsync(CancellationToken cancellationToken)
        {
            ...
            if (string.IsNullOrWhiteSpace(grpcBinding) || string.IsNullOrWhiteSpace(modelPath))
            {
                Console.WriteLine($"ERROR: Invalid arguments.");
                Console.WriteLine($"USAGE: {Assembly.GetEntryAssembly().GetName()} --grpcBinding <GRPC_BINDING> --model <MODEL_PATH>");
                Environment.Exit(-1);
            }
            ...
            var moduleExtension = new MediaGraphExtensionService(_logger, modelPath);
